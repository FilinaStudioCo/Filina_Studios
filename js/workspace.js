...
function editFinance(id) {
  ...
  if (!f) return;
  openModalHTML('<h2>✏️ Edit Finance</h2><div class="modal-form"><input id="ef0" type="text" value="' + f.name + '" placeholder="Name"/><select id="ef1">' + opts(["Expense","Income"], f.type) + '</select><select id="ef2">' + opts(["Equipment","Software","Hosting","Marketing","Salary","Sponsorship","Other"], f.category) + '</select><input id="ef3" type="number" value="' + f.amount + '"/><select id="ef4">' + opts(["Planned","Pending","Paid"], f.status) + '</select><input id="ef5" type="text" value="' + f.date + '" placeholder="YYYY-MM"/><div class="modal-actions"><button class="add-btn-lg" onclick="saveEdit(\'finance\',\'' + id + '\')">Save</button><button class="btn-cancel" onclick="closeModal()">Cancel</button></div></div>');
}
...
function renderWiki() {
  ...
  if (!el) return;
  el.innerHTML = '<div class="wiki-section"><h3>🏢 About Filina Studios</h3><div class="wiki-links"><span class="wiki-link">Our Story</span><span class="wiki-link">Mission & Vision</span><span class="wiki-link">Team Bios</span><span class="wiki-link">Brand Guidelines</span></div></div><div class="wiki-section"><h3>🎙️ Podcast Production</h3><div class="wiki-links"><span class="wiki-link">Recording Checklist</span><span class="wiki-link">Editing Workflow</span><span class="wiki-link">Publishing Checklist</span><span class="wiki-link">Episode Planning</span><span class="wiki-link">Guest Management</span></div></div><div class="wiki-section"><h3>📣 Marketing & Growth</h3><div class="wiki-links"><span class="wiki-link">Social Media SOP</span><span class="wiki-link">Content Approval</span><span class="wiki-link">Hashtag Library</span></div></div><div class="wiki-section"><h3>💰 Finance & Admin</h3><div class="wiki-links"><span class="wiki-link">Expense Reporting</span><span class="wiki-link">Budget Overview</span></div></div><div class="wiki-section"><h3>👥 HR & Team</h3><div class="wiki-links"><span class="wiki-link">Onboarding Checklist</span><span class="wiki-link">Team Policies</span></div></div><div class="wiki-section"><h3>🔧 IT & Tools</h3><div class="wiki-links"><span class="wiki-link">Tools We Use</span><span class="wiki-link">Password Policy</span></div></div>';
}
...
function openModal(type) {
  var templates = {
    ...
    contact:   '<h2>🤝 New Contact</h2><div class="modal-form"><input id="mf0" type="text" placeholder="Full Name"/><select id="mf1"><option>Guest</option><option>Sponsor</option><option>Partner</option><option>Listener</option><option>Press</option></select><input id="mf2" type="text" placeholder="Company"/><input id="mf3" type="email" placeholder="Email"/><select id="mf4"><option>New Lead</option><option>Contacted</option><option>In Conversation</option><option>Confirmed</option></select><div class="modal-actions"><button class="add-btn-lg" onclick="addContact()">Save</button><button class="btn-cancel" onclick="closeModal()">Cancel</button></div></div>',
